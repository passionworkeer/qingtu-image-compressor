...
视频、文档等非图片文件不复制到结果目录，CSV 记录为“非图片已跳过”，不算异常，也不计入压缩节省量。图片仍按内容识别，无后缀或错后缀图片不会仅因文件名被忽略。解码器能确认是图片、但暂时不能压缩的格式会保留原件并提示；仅有 RAW/SVG 等后缀、内容却无法确认为图片的文件会跳过，避免伪装或损坏文件混入结果。
...
扫描会忽略 `.Spotlight-V100`、`.Trashes`、`.fseventsd`、`$RECYCLE.BIN` 等系统索引/回收站目录，以及 `.DS_Store` 等系统文件。损坏、截断、像素数异常或处理期间仍在变化的图片会写入报告，但不会复制到结果目录；明确的外接盘断开错误会停止任务，避免继续反复读取失联设备。文件夹模式不允许把导出位置选在源文件夹内部，防止下次运行递归处理旧结果。
...
界面测试会短暂打开 Tk 窗口，覆盖 Tcl 格式的文件夹/单图拖入、跨目录多图拒绝、开始按钮、后台处理、620–880 像素窗口高度和结果状态。核心测试覆盖 20 MB 大图、100MP 有效图片、28 层目录、无后缀及错后缀图片、原件哈希、透明度、旋转、正常/损坏 ICC、截断单图和动图、HEIC/AVIF/WebP/TIFF、系统目录、云盘 reparse、扫描后链接替换、源文件变化、外接盘断开、exFAT 发布回退、同名冲突、磁盘满停止及取消。
...
为兼容 100MP 级专业相机，单帧安全上限设为 120,000,000 像素；每一帧/页都会检查。超过上限的文件会作为异常跳过，避免伪造的小文件在解码时占用不可控内存。无效 ICC 会在确实需要重编码时移除并写入报告，像素仍可正常解码的照片会继续处理。
